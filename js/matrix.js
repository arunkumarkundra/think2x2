...
function generateAxes(template, plotArea, xAxisName, yAxisName) {
    ...
    svg += `<text x="${plotArea.x + plotArea.width + axisExtend + 15}" y="${midY + 5}" 
        font-family="Inter, sans-serif" 
    ...
    // Y-axis label
    svg += `<text x="${midX}" y="${plotArea.y - axisExtend - 10}" 
        text-anchor="middle"
        font-family="Inter, sans-serif" 
        font-size="${template.axisFontSize}" 
        font-weight="${fontWeight}"
        fill="${template.axisColor}">${yAxisName}</text>`;
...
/**
 * Generate axis value labels (0, 50, 100)
 */
function generateAxisLabels(template, plotArea) {
    const midX = plotArea.x + plotArea.width / 2;
    const midY = plotArea.y + plotArea.height / 2;
    const fontSize = template.labelFontSize - 2;
    const offset = 8;
    
    let svg = '<g class="axis-labels" opacity="0.6">';
    
    // X-axis labels
    svg += `<text x="${plotArea.x}" y="${midY + fontSize + offset}" 
        text-anchor="middle" font-family="Inter, sans-serif" 
        font-size="${fontSize}" fill="${template.textColor}">0</text>`;
    
    svg += `<text x="${midX}" y="${midY + fontSize + offset}" 
        text-anchor="middle" font-family="Inter, sans-serif" 
        font-size="${fontSize}" fill="${template.textColor}">50</text>`;
    
    svg += `<text x="${plotArea.x + plotArea.width}" y="${midY + fontSize + offset}" 
        text-anchor="middle" font-family="Inter, sans-serif" 
        font-size="${fontSize}" fill="${template.textColor}">100</text>`;
    
    // Y-axis labels
    svg += `<text x="${midX - fontSize - offset}" y="${plotArea.y + plotArea.height + 5}" 
        text-anchor="end" font-family="Inter, sans-serif" 
        font-size="${fontSize}" fill="${template.textColor}">0</text>`;
    
    svg += `<text x="${midX - fontSize - offset}" y="${midY + 5}" 
        text-anchor="end" font-family="Inter, sans-serif" 
        font-size="${fontSize}" fill="${template.textColor}">50</text>`;
    
    svg += `<text x="${midX - fontSize - offset}" y="${plotArea.y + 5}" 
        text-anchor="end" font-family="Inter, sans-serif" 
        font-size="${fontSize}" fill="${template.textColor}">100</text>`;
...
function generateFooter(template, dims) {
    const y = dims.height - dims.footerHeight + 20;
    ...
    return `<g class="footer">
        ...
        <text x="${dims.padding}" y="${y}" 
            font-family="Inter, sans-serif" 
            font-size="${template.footerFontSize}" 
            fill="${template.textColor}"
            opacity="0.7">Generated by: Think2x2</text>
        
        <text x="${dims.padding}" y="${y + 15}" 
            font-family="Inter, sans-serif" 
            font-size="${template.footerFontSize}" 
            fill="${template.textColor}"
            opacity="0.7">Generated on: ${timestamp}</text>
        
        <text x="${dims.padding}" y="${y + 30}" 
            font-family="Inter, sans-serif" 
            font-size="${template.footerFontSize}" 
            fill="${template.textColor}"
            opacity="0.7">Visit: https://think2x2.com</text>
    </g>`;
}
